...
import { AdminSession, CustomerProfile, Order, OrderStatus, MenuItem, OfferCard, WalletTransaction } from '../types';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ session, onSessionChange, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'orders' | 'wallets' | 'menu' | 'offers' | 'dashboard' | 'notifications'>('orders');
  ...
  return (
    <div className="fixed inset-0 z-[200] overflow-y-auto bg-[#08080d] text-white animate-fade-in">
      ...
      <div className="pb-12">
        ...
        {(activeTab === 'menu' || activeTab === 'offers') && session.role !== 'staff' && (
          <AdminMenu
            session={session}
            menuItems={menuItems}
            offers={offers}
