  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeServerOrder(
      ...
      (order) => {
        ...
        setPastOrders((currentOrders) => currentOrders.map((currentOrder) => (currentOrder.id === order.id ? order : currentOrder)));
      },
      () => undefined,
    );
    const statusPoll = window.setInterval(() => {
      void getServerOrders()
        .then((orders) => {
          const updatedOrder = orders.find((order) => order.id === latestOrder.id);
