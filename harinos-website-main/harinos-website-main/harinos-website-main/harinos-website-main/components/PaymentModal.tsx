...
const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  const handleInitiateUPIPayment = (upiUrl: string) => {
    ...
    // Wait exactly 12 seconds, then show order placed
    setTimeout(() => {
      setIsWaiting(false);
      ...
      >
        ...
        {isWaiting && (
          <div className="absolute inset-0 z-[230] flex flex-col items-center justify-center bg-white/95 p-6 text-center animate-fade-in">
            <div className="h-16 w-16 animate-spin rounded-full border-4 border-slate-200 border-t-red-600 mb-6" />
            <h3 className="text-xl font-display font-black text-slate-900">Please wait...</h3>
            <p className="mt-2 text-xs text-slate-500 max-w-xs leading-relaxed font-bold">
              Waiting for order confirmation. Do not close or refresh this screen.
            </p>
          </div>
        )}
